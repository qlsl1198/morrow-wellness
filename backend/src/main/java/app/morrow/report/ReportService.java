...
public class ReportService {
 ...
 public WeeklyReport generateWeeklyReport(){var weekAgo=OffsetDateTime.now().minusDays(7);var checkIns=checkInRepository.findByRecordedAtAfterOrderByRecordedAtDesc(weekAgo);var totalCount=checkIns.size();var statusCounts=checkInRepository.countByStatusAfter(weekAgo).stream().collect(Collectors.toMap(arr->(CheckIn.Status)arr[0],arr->(Long)arr[1]));var causeCounts=checkInRepository.countByCauseAfter(weekAgo).stream().collect(Collectors.toMap(arr->(CheckIn.Cause)arr[0],arr->(Long)arr[1]));var topStatus=statusCounts.entrySet().stream().max(Map.Entry.comparingByValue()).map(e->e.getKey().name()).orElse(null);var topCause=causeCounts.entrySet().stream().max(Map.Entry.comparingByValue()).map(e->e.getKey().name()).orElse(null);var okCount=statusCounts.getOrDefault(CheckIn.Status.OK,0L);var notOkCount=totalCount-okCount;var improvement=totalCount>0?(double)okCount/totalCount*100:0.0;var patterns=generatePatterns(topStatus,topCause,improvement);var insights=generateInsights(totalCount,topStatus,topCause,improvement);return new WeeklyReport(totalCount,topStatus,topCause,improvement,patterns,insights);}
