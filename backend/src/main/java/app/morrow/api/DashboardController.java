...
public class DashboardController {
 private final DashboardService service; private final RequestUserResolver users; public DashboardController(DashboardService service,RequestUserResolver users){this.service=service;this.users=users;}
 @GetMapping("/dashboard") DashboardResponse dashboard(@RequestParam(defaultValue="default-user")String userId){var data=service.getDashboard(users.resolve(userId));return new DashboardResponse(data.wellnessLoad(),data.score(),MetricsResponse.from(data.metrics()),data.timelines().stream().map(TimelineResponse::from).collect(Collectors.toList()),data.recommendation()!=null?RecommendationResponse.from(data.recommendation()):null,"의료 진단이 아닌 일상 웰니스 분석입니다.");}
