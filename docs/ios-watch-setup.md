...
4. 계정 ID로 로그인한 뒤 연결 상태를 확인합니다.
5. iPhone Morrow → 설정에서 6자리 연결 코드를 만들고, 로그인한 웹 Morrow → 설정 → 기기 연결에 입력합니다.

연결 코드는 로그인 수단이 아닙니다. 코드는 잠시 후 만료되지만 연결된 계정·기기 관계는 로그아웃하거나 설정에서 해당 기기를 해제하기 전까지 유지됩니다.
...
- iPhone 대시보드: 웹 콘솔과 동일한 다크·청록 디자인, 회복 게이지, 근거 칩, 수면·HRV·안정 심박·걸음, 개인화 행동 추천과 `지금 바로 실행`
...
- Watch 회복 실행: 추천 종류와 시간에 맞는 호흡·걷기·스트레칭·집중·화면 휴식 흐름, 진행 링, 햅틱과 체감 결과 기록

HealthKit 상세는 최근 수면의 시작·종료와 코어·깊은·REM·깨어있음 시간, 최근 운동의 종류·구간·에너지·거리·평균/최대 심박·추정 강도를 포함합니다. 강도는 의료 판단이 아닌 심박과 분당 활동 에너지 기반 참고값입니다.
